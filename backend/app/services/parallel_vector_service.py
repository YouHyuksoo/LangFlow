...
class ParallelVectorService:
    ...
    def _load_performance_settings(self):
        ...
        try:
            # 벡터화 성능 설정을 통합 설정에서 가져오기
            from .settings_service import settings_service
            performance_settings = settings_service.get_section_settings("performance")
            
            # 성능 설정에서 가져오기
            self.max_concurrent_embeddings = performance_settings.get("maxConcurrentEmbeddings", settings.MAX_CONCURRENT_EMBEDDINGS)
            self.max_concurrent_chunks = performance_settings.get("maxConcurrentChunks", settings.MAX_CONCURRENT_CHUNKS)
            self.embedding_pool_size = performance_settings.get("embeddingPoolSize", settings.EMBEDDING_POOL_SIZE)
            self.connection_pool_size = performance_settings.get("connectionPoolSize", settings.CONNECTION_POOL_SIZE)
